...
!!! note "Exception: ephemeral mode"
    Agents started with `--ephemeral` (or `KH_EPHEMERAL=true`) invert this: they read
    configuration from system environment variables and never read or write a `.env`
    file. This is for disposable instances — containers and rented cloud GPUs — which
    have no persistent filesystem to carry a `.env`, and where writing `KH_CLAIM_CODE`
    to disk would expose a live registration credential to whoever operates the machine.

    It is opt-in precisely because the default behavior protects co-located agents from
    inheriting the backend's `KH_CONFIG_DIR`/`KH_DATA_DIR`.
...
**Important**: By default the agent does NOT read from system environment variables, to avoid conflicts when running on the same host as the backend. All configuration is handled through the `.env` file and command-line flags.

The exception is **ephemeral mode** (`--ephemeral` / `KH_EPHEMERAL=true`), where the order becomes:
